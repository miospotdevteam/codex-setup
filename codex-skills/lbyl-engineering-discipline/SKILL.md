...
description: "Engineering discipline and verification layer for coding tasks that touch source files. Use for writing, editing, fixing, refactoring, porting, migrating, reviewing, or debugging code. Enforces exploration before editing, blast-radius checks on shared changes, no type-safety shortcuts, required verification, no silent scope cuts, and no stopping mid-plan. Do NOT use for pure research, documentation-only queries, or conversations with no code changes."
---
